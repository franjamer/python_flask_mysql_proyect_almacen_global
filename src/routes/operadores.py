...
#         cursor = conn.cursor()
#         sql = "INSERT INTO operadores (codigo_operador, nombre_completo, puesto) VALUES (%s, %s, %s)"
#         operadores = (codigo_operador, nombre_completo, puesto)
#         cursor.execute(sql,operadores)
#         conn.commit()
#         cursor.close()
#         conn.close()
#     return redirect(url_for('home_bp.operadores'))

# @operadores_bp.route('/operadores/<int:id>', methods = [POST])
# def delete(id):
#     if session.get('rol') != 'admin':
#         return redirect(url_for('home_bp.menu'))
#     conn = db.get_connection()
...
#     cursor = conn.cursor()
#     sql = "UPDATE operadores SET codigo_operador = %s, nombre_completo = %s, puesto= %s  WHERE id_operador = %s"
#     cursor.execute(sql, (codigo_operador, nombre_completo, id))
#     conn.commit()
#     cursor.close()
#     conn.close()
#     return redirect(url_for('home_bp.operadores'))